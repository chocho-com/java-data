...
/**
 * Servlet implementation class UpdateEmpInf
 */
...
public class UpdateEmp extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		 ...
		 try {
				int count = new EmpService().UpdateEmpInf(emp);
				 if(count==0) {
					 response.getWriter().print("<script type='text/javascript'>alert('更新失败！');"
								+ "window.history.back();</script>");
				 }
				 else {
					 response.getWriter().print("<script type='text/javascript'>alert('更新成功！');"
								+ "window.history.back();</script>");
				 }
			} catch (Exception e) {
